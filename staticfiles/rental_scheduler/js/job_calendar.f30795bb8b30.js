    ...
    class JobCalendar {
        ...
        openEventDetailsDialog(info) {
            try {
                ...
                const eventId = info.event.extendedProps?.job_id || info.event.id;
                
                // Extract numeric ID from event ID (e.g., "job-3" -> "3")
                const jobId = eventId.toString().replace(/^job-/, '');
                
                if (jobId) {
                    console.log('JobCalendar: Loading modal for job ID:', jobId, '(extracted from:', eventId, ')');
                    
                    // Direct fetch approach (more reliable than HTMX for this use case)
                    fetch(`/jobs/${jobId}/detail/`)
                        ...
                        .then(html => {
                            console.log('JobCalendar: Modal HTML received, inserting into DOM');
                            document.body.insertAdjacentHTML('beforeend', html);
                        })
                        .catch(error => {
                            console.error('JobCalendar: Error loading job modal:', error);
                            // Show a simple alert as fallback
                            alert(`Error loading job details: ${error.message}`);
                        });
                } else {
                    console.error('JobCalendar: No job ID found in event data');
                    ...
                    <div style="display: flex; flex-direction: column; gap: 32px;">
                        ...
                        <div style="background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden;">
                            ...
                            <div style="padding: 24px;">
                                <div class="grid-responsive" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                                    <div>
                                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 14px;">Start Date</div>
                                        <div style="color: #6b7280; font-size: 14px;">${formatDateTime(event.start).split(' at ')[0]}</div>
                                    </div>
                                    <div>
                                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 14px;">Start Time</div>
                                        <div style="color: #6b7280; font-size: 14px;">${formatDateTime(event.start).split(' at ')[1] || 'N/A'}</div>
                                    </div>
                                    <div>
                                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 14px;">End Date</div>
                                        <div style="color: #6b7280; font-size: 14px;">${event.end ? formatDateTime(event.end).split(' at ')[0] : 'Not set'}</div>
                                    </div>
                                    <div>
                                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 14px;">End Time</div>
                                        <div style="color: #6b7280; font-size: 14px;">${event.end ? formatDateTime(event.end).split(' at ')[1] || 'N/A' : 'Not set'}</div>
                                    </div>
                                    <div style="grid-column: 1 / -1;">
         ...
         */
        showError(message) {
            // Simple error display - you can enhance this as needed
